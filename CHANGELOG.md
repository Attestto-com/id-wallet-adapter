...
- **Hardened per code review:** (1) auth keys published as `publicKeyMultibase` (the norm for `Ed25519VerificationKey2020` / `Multikey` / did:key) are now matched, not just `publicKeyJwk`, via a zero-dep base58-btc + multicodec decode, without which `verifyAuth` was non-functional for most real DIDs; (2) signed fields containing control characters are rejected (`MALFORMED_FIELDS`) to prevent newline/field-shift injection in the canonical payload; (3) signatures are length/format-validated, so a DER-encoded ECDSA signature (WebCrypto needs raw IEEE-P1363 r‖s) is rejected with `SIGNATURE_FORMAT_INVALID` instead of a silent failure; (4) binding/freshness failures now return **before** any signature verification or DID resolution; (5) optional `isNonceUsed` hook enforces single-use so a captured-but-fresh response cannot be replayed within the window (`NONCE_REPLAYED`).
...
### Notes
- Additive only, with no breaking changes. Wallet implementations that ignore `trustedIssuers` continue to work; sites that don't set it get the previous behavior (wallet shows all identities).
...
- `requestSignature()` for nonce-based document signing, with timeout and consent flow.
...
### Added
- `discoverWallets()` for nonce-based browser event discovery of credential wallet extensions.
- `registerWallet()` for wallet-side registration in content scripts.
- `verifyPresentation()` for VP validation with DID resolution, signature verification, and issuer trust checks.
